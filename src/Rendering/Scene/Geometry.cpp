...
void Geometry::CreateCube(std::vector<VertexPositionColor>& vertices, std::vector<unsigned short>& indices, const Vector4& color, const Vector3& position, const Vector3& dimensions)
{
    ...
    vertices[0].position = Vector3(minVertex.x, minVertex.y, minVertex.z);
    vertices[1].position = Vector3(maxVertex.x, minVertex.y, minVertex.z);
    vertices[2].position = Vector3(maxVertex.x, maxVertex.y, minVertex.z);
    vertices[3].position = Vector3(minVertex.x, maxVertex.y, minVertex.z);
    vertices[4].position = Vector3(minVertex.x, minVertex.y, maxVertex.z);
    vertices[5].position = Vector3(maxVertex.x, minVertex.y, maxVertex.z);
    vertices[6].position = Vector3(maxVertex.x, maxVertex.y, maxVertex.z);
    vertices[7].position = Vector3(minVertex.x, maxVertex.y, maxVertex.z);
...
{
    vertices.emplace_back(Vector3(-0.5f, 0.0f, 0.5f), Vector2(0, 0), Vector3(0, 1, 0), Vector3(1, 0, 0));
    vertices.emplace_back(Vector3(0.5f, 0.0f, 0.5f), Vector2(1, 0), Vector3(0, 1, 0), Vector3(1, 0, 0));
    vertices.emplace_back(Vector3(-0.5f, 0.0f, -0.5f), Vector2(0, 1), Vector3(0, 1, 0), Vector3(1, 0, 0));
    vertices.emplace_back(Vector3(0.5f, 0.0f, -0.5f), Vector2(1, 1), Vector3(0, 1, 0), Vector3(1, 0, 0));
...
void Geometry::CreateWireCapsule(std::vector<VertexPosition>& vertices, const Vector3& base, const Vector3& x, const Vector3& y, const Vector3& z, double radius, double HalfHeight, int NumSides)
{
    ...
    HalfHeight *= zScale;
    capsuleRadius = Math::Clamp(capsuleRadius, 0.0, HalfHeight);
