...
void ResourceBrowserPanel::Render()
{
    ...
    if (ResourceInfoRegistry::GetInstance().IsResourcesInfoLoaded())
    {
        ...
        if (showPatchPopup && resource && resource->IsResourceLoaded())
        {
            ...
            if (!textList->IsResourceDeserialized())
            {
            ...
            if (!filePath.empty())
            {
                ...
                if (textList->PatchResourceLibrary())
                {
                    Logger::GetInstance().Log(Logger::Level::Info, std::format("Patched resource {}", resource->GetName()));
                }
                else
                {
                    Logger::GetInstance().Log(Logger::Level::Error, std::format("Failed to patch resource {}", resource->GetName()));
                }
            }
        }
...
void ResourceBrowserPanel::RenderContextMenu(ResourceNode& resourceNode)
{
    ...
    static std::string patchResourceLabel = std::format("{} Patch Resource Library (Import JSON & Write Back)", ICON_MDI_CONTENT_SAVE);
...
void ResourceBrowserPanel::AddChildren(ResourceNode& parentNode, const std::string& parentPath)
{
    ...
    for (auto it = resourcesInfo.begin(); it != resourcesInfo.end(); ++it)
    {
        ...
        if (resourceInfo.resourceID.starts_with("[" + parentPath) || resourceInfo.resourceID.starts_with("[[" + parentPath))
        {
...
void ResourceBrowserPanel::BatchImportTeliFiles(const std::string& inputFolder)
{
    ...
    for (const auto& jsonFile : jsonFiles)
    {
        ...
        {
            jsonFileName = jsonFileName.substr(0, jsonFileName.length() - 9);
        }
        bool found = false;
