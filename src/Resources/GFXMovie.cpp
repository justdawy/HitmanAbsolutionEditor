...
void GFXMovie::Parse(void* gfxResource)
{
	...
	const unsigned int fileNameCount = ResourceUtility::CalculateArrayElementsCount(fileNamesStartAddress, fileNamesEndAddress, 0x8);
	...
	const unsigned int fileContentCount = ResourceUtility::CalculateArrayElementsCount(fileContentsStartAddress, fileContentsEndAddress, 0xC);
	...
	for (unsigned int i = 0; i < fileContentCount; i++)
	{
		...
		const unsigned int byteCount = ResourceUtility::CalculateArrayElementsCount(dataStartAddress, dataEndAddress, 0x1);
