...
void PackageList::Parse(void* packageListData)
{
	...
	const unsigned int folderCount = ResourceUtility::CalculateArrayElementsCount(foldersStartAddress, foldersEndAddress, 0x14);
	...
	for (unsigned int i = 0; i < folderCount; i++)
	{
		...
		const unsigned int sectionIndexCount = ResourceUtility::CalculateArrayElementsCount(sectionsStartAddress, sectionsEndAddress, 0x18);
		...
		for (unsigned int j = 0; j < sectionIndexCount; j++)
		{
			...
			const unsigned int sceneResourceIDIndexCount = ResourceUtility::CalculateArrayElementsCount(sceneResourceIDsStartAddress, sceneResourceIDsEndAddress, 0x8);
			...
			const unsigned int resourceCount = ResourceUtility::CalculateArrayElementsCount(resourcesStartAddress, resourcesEndAddress, 0x8);
